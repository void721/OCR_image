...
from llava.model.builder import load_pretrained_model_org, load_pretrained_model_llavaprumerge
...
def visualize_attention_high_resolution(multihead_attention, output_path="atten_map_1.png", title="Layer 5"):
...
def eval_model(args):
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model_org(
    ...
    if args.attention_map is True:
        attention_with_output = prompt+outputs
        
        input_ids_with_output = (
            tokenizer_image_token(attention_with_output, tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt")
            .unsqueeze(0)
            .cuda()
        )
        
        with torch.inference_mode():
            output_ids_with_output = model.generate(
                input_ids_with_output,
                images=images_tensor,
                image_sizes=image_sizes,
                do_sample=True if args.temperature > 0 else False,
        ...
        outputs_attention_with_output = [output_ids_with_output['attentions']]
         # outputs_attention_with_output:[1,32,1,32,2254,2254]
        import pdb
        pdb.set_trace()
        ...
        for i in outputs_attention_with_output:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--attention_map", type=str, default=False)
