...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            #如果图片是none -- 直接返回input_ids，不需要decode image
            return input_ids, position_ids, attention_mask, past_key_values, None, labels

        if type(images) is list or images.ndim == 5:
            if type(images) is list:
                images = [x.unsqueeze(0) if x.ndim == 3 else x for x in images]
            concat_images = torch.cat([image for image in images], dim=0) #拼接起来
            # concat_images.shape -> torch.Size([5, 3, 336, 336])
            image_features = self.encode_images(concat_images)
            # image_features.shape -> torch.Size([5, 576, 4096])
            split_sizes = [image.shape[0] for image in images]
            # split_sizes -> [5]
            
            image_features = torch.split(image_features, split_sizes, dim=0)
            # (Pdb) len(image_features)
            # 1
            # (Pdb) len(image_features[0])
            # 5
            # (Pdb) len(image_features[0][0])
            # 576
            # (Pdb) len(image_features[0][0][0])
            # 4096
